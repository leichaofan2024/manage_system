class ExaminationAwardsController < ApplicationController
  ...
  def index
    ...
    if (current_user.has_role? :organsadmin) || (current_user.has_role? :workshopadmin)
      @examinations = ExaminationAward.where(:upload_year => @year,:upload_month => @month,:车间 => current_user.name).group_by{|x| x.工资编号}
      @export_examinations = ExaminationAward.where(:upload_year => @year,:upload_month => @month,:车间 => current_user.name).group_by{|x| x.工资编号}
    else
      @examinations = ExaminationAward.where(:upload_year => @year,:upload_month => @month).group_by{|x| x.工资编号}
      @export_examinations = ExaminationAward.where(:upload_year => @year,:upload_month => @month).group_by{|x| x.工资编号}
    end
