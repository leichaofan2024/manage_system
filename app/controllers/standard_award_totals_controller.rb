class StandardAwardTotalsController < ApplicationController
  ...
  def index
    ...
    if (current_user.has_role? :organsadmin) || (current_user.has_role? :workshopadmin)
      @standard_award_totals = StandardAwardTotal.where(:upload_year => @year,:upload_month => @month,:科室车间 => current_user.name).group_by{|x| x.科室车间}
      @export_standard_award_totals = StandardAwardTotal.where(:upload_year => @year,:upload_month => @month,:科室车间 => current_user.name).group_by{|x| x.科室车间}
    else 
