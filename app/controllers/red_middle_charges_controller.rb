class RedMiddleChargesController < ApplicationController
  ...
  def index
    ...
    if (current_user.has_role? :organsadmin) || (current_user.has_role? :workshopadmin)
      @red_middle_charges = RedMiddleCharge.where(:upload_year => @year,:upload_month => @month,:部门 => current_user.name).group_by{|x| x.工资号}
      @export_red_middle_charges = RedMiddleCharge.where(:upload_year => @year,:upload_month => @month,:部门 => current_user.name).group_by{|x| x.工资号}
    else
