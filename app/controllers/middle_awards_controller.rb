  ...
  layout 'home'
    ...
    def index
      ...
      if (current_user.has_role? :organsadmin) || (current_user.has_role? :workshopadmin)
        @middle_awards = MiddleAward.where(:upload_year => @year,:upload_month => @month,:部门 => current_user.name).group_by{|x| x.工资号}
        @export_middle_awards = MiddleAward.where(:upload_year => @year,:upload_month => @month,:部门 => current_user.name).group_by{|x| x.工资号}
      else
