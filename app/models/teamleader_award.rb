class TeamleaderAward < ApplicationRecord
  ...
  def self.import_form(file, upload_time)
    ...
    form_headers = TeamleaderAward.column_names
    import_form_headers.each do |fh|
      if !form_headers.include?(fh)
        message[:head] = "所上传的班组长返奖明细表表头为‘#{fh}’的字段与系统不匹配，请核对后再上传！"
      end
    end
    ...
    if message.blank?
      (2..spreadsheet.last_row).each do |n|
        ...
        TeamleaderAward.create(row_hash)
