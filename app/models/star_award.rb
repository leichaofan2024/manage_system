class StarAward < ApplicationRecord
  ...
  def self.import_form(file, upload_time)
    ...
    form_headers = StarAward.column_names
    import_form_headers.each do |fh|
      if !form_headers.include?(fh)
        message[:head] = "所上传的星级岗奖励总明细表表头为‘#{fh}’的字段与系统不匹配，请核对后再上传！"
      end
    end
    year_month_already_import = StarAward.pluck(:upload_year,:upload_month).uniq
