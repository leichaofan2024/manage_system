class RectificationAward < ApplicationRecord
  ...
  def self.import_form(file, upload_time)
    ...
    form_headers = RectificationAward.column_names
    import_form_headers.each do |fh|
      if !form_headers.include?(fh)
        message[:head] = "所上传的整改返奖明细表表头为‘#{fh}’的字段与系统不匹配，请核对后再上传！"
      end
    end
    year_month_already_import = RectificationAward.pluck(:upload_year,:upload_month).uniq
    ...
    if message.blank?
      (2..spreadsheet.last_row).each do |n|
        ...
        RectificationAward.create(row_hash)
