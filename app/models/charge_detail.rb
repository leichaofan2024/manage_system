class ChargeDetail < ApplicationRecord
  ...
  def self.import_form(file, upload_time)
    ...
    form_headers = ChargeDetail.column_names
    import_form_headers.each do |fh|
      if !form_headers.include?(fh)
        message[:head] = "所上传的考核扣款明细表表头为‘#{fh}’的字段与系统不匹配，请核对后再上传！"
      end
    end
    year_month_already_import = ChargeDetail.pluck(:upload_year,:upload_month).uniq
